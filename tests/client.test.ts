...
import { QdrantClient, createQdrantClient, createLocalQdrantClient, createQdrantCloudClient } from '../src/client.ts'
import {
  QdrantError,
...
Deno.test('QdrantClient - factory functions', () => {
  // createQdrantClient
  ...
  assertEquals(client2.getInfo().url, 'http://localhost:6333')

  const client3 = createLocalQdrantClient(6334)
  assertEquals(client3.getInfo().url, 'http://localhost:6334')

  // createQdrantCloudClient
  const client4 = createQdrantCloudClient('https://xyz.qdrant.tech', 'api-key')
  assertEquals(client4.getInfo().url, 'https://xyz.qdrant.tech')
  assertEquals(client4.getInfo().hasApiKey, true)
})
